...
function Test-DismVersion {
    ...
    .DESCRIPTION
        Verifies that DISM servicing support is available when required by configuration.
        The validation accepts either DISM PowerShell cmdlets or dism.exe because the DISM
        module automatically uses cmdlets first and falls back to dism.exe.
    ...
    $dismCmdlets = @(
        Get-Command -Name 'Get-WindowsImage' -CommandType Cmdlet -ErrorAction SilentlyContinue
        Get-Command -Name 'Mount-WindowsImage' -CommandType Cmdlet -ErrorAction SilentlyContinue
        Get-Command -Name 'Add-AppxProvisionedPackage' -CommandType Cmdlet -ErrorAction SilentlyContinue
    )
    $hasDismExe = ($null -ne $dismCommand)
    $hasDismCmdlets = ($dismCmdlets.Count -gt 0)
    $fileVersion = $null
    $dismExePath = $null

    if ($hasDismExe) {
        $dismExePath = $dismCommand.Source
    }

    if ($hasDismExe -and $dismExePath -and (Test-PathExists -Path $dismExePath -PathType Leaf)) {
        $fileVersion = [System.Diagnostics.FileVersionInfo]::GetVersionInfo($dismExePath).FileVersion
    }

    $passed = ((-not $requiresDism) -or $hasDismExe -or $hasDismCmdlets)
    $details = [ordered]@{
        RequireDISM     = $requiresDism
        DismExeFound    = $hasDismExe
        DismExePath     = $dismExePath
        DismExeVersion  = $fileVersion
        DismCmdletFound = $hasDismCmdlets
        DismCmdlets     = @($dismCmdlets.Name)
    }
    $message = 'DISM support: dism.exe found={0}; DISM cmdlets found={1}.' -f $hasDismExe, $hasDismCmdlets
    $result = New-ValidationResult -Name 'DismVersion' -Passed $passed -Message $message -Details $details
    Write-ValidationResultLog -Result $result
